...
def calculate_chrf_score_for_each_sentence_in_data(
    hypotheses: List[str],
    references: List[str],
    ...
    ast_dataset = pd.read_csv(ast_dataset_filename, sep="\t").filter(
...
def get_reference_and_hypothesis_strings_from_datafile(
    filename: str,
...
def test_get_reference_and_hypothesis_strings_from_datafile():
    assert ["Hello , goodbye", "Goodbye ! or"], [
        "Hello , goodbye",
        "Hello , goodbye",
...
if __name__ == "__main__":
    ...
    (
        references_list,
        hypotheses_list,
    ) = get_reference_and_hypothesis_strings_from_datafile(fairseq_file)
    asr_ref, asr_hypos = get_reference_and_hypothesis_strings_from_datafile(
        args.asr_output
    )
