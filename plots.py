...
def get_data(filename="fairseq/wmt19.en-de.joined-dict.ensemble/dict.en.txt", csv_filename="covost_wer_0_5.csv"):
...
def create_probs_plot(dataframe_row, np_array, index2word, image_folder, show_transcript=True):
    ...
    for row_number, prob_at_timestep in enumerate(np_array):
        ...
        if row_number <= 5:
            ax.set_title(f"prefix: {' '.join(prefix_at_timestep[:row_number])}", {'fontsize':10})
        else:
            ax.set_title(f"prefix: [...]{' '.join(prefix_at_timestep[row_number-5:row_number])}", {'fontsize':10})
    ...
    #plt.show()
    plt.savefig(f"{image_folder}/{transcript}_{correct_transcript}_tok_k_probs.png", bbox_inches="tight")
    plt.close(fig)


def choose_row_and_numpy_file(dataframe, np_folder, index2word, wer, save_folder, to_keep=15):
    ...
    plt.hist(relevant_data_rows.WER.clip(upper=100).values, bins='auto', rwidth=0.9, color='#0504aa')
    plt.grid()
    plt.xlabel('WER')
    ...
    print(len(relevant_data_rows))
    sorted_relevant_data_rows = relevant_data_rows.sort_values('WER', ascending=False)[:to_keep]
    ...
    for _, data_row in sorted_relevant_data_rows.iterrows():
        ...
        create_probs_plot(data_row, np.squeeze(np_array[[sample_index]], 0), index2word, save_folder)
        create_probs_plot(data_row, np.squeeze(np_array_orig[[sample_index]], 0), index2word, save_folder+"_the_original", show_transcript=False)


# load vocabulary into fairseq dictionary
dictionary, data = get_data(csv_filename="covost_wer_few.csv")
# plot probabilities
choose_row_and_numpy_file(data, "expert_probs_kd_on_translations_few", dictionary, 0, "asrkd_sample_100_above_0", to_keep=100)
...
plt.ylabel('Frequency')
plt.show()
plt.hist(data.WER.clip(upper=100).values, bins='auto', rwidth=0.9, color='#0504aa', density=True)
plt.grid()
plt.xlabel('WER')
plt.ylabel('Density')
plt.rc('font', size=15)
