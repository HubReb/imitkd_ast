...
from fairseq.criterions.fairseq_sequence_criterion import FairseqSequenceCriterion
...
class SequenceRiskCriterion(FairseqSequenceCriterion):
    ...
    def __init__(self, args, dst_dict):
        ...
        self.seq_score_alpha = args.seq_score_alpha
    ...
    def prepare_sample_and_hypotheses(self, model, sample, hypos):
        ...
        if self.seq_score_alpha == 0: #BLEU only
        ...
        elif self.seq_score_alpha == 1: #SIM only
            # compute SIM cost for each hypothesis
            hypos = self.add_sim_to_hypotheses(sample, hypos)
            cost = torch.FloatTensor([
                scale_scores([100 - h['sim']*100 for h in hypos_i])
                for hypos_i in hypos
            ])
        else:
            hypos = self.add_bleu_to_hypotheses(sample, hypos)
            hypos = self.add_sim_to_hypotheses(sample, hypos)
            cost = torch.FloatTensor([
                scale_scores([(100 - h['sim'] * 100) * self.seq_score_alpha +
                (1 - self.score_alpha) * (100 - h['bleu']) for h in hypos_i])
            ])
