...
@register_criterion(
    "imit_asr_input_nmt_generated_dataset", dataclass=ImitKDConfig
)
class ImitKDGeneratedTranscriptions(FairseqCriterion):
    def __init__(
            self,
            task,
            expert,
    ...
    def generate_imit_batch(self, student: Callable, sample: Dict) -> Tuple[Dict, torch.IntTensor, List[int]]:
        ...
        with torch.no_grad():
            ...
            sample["net_input"]["src_lengths"] = torch.tensor(lengths)
            max_length = max(
