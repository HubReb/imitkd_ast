...
def imit_kd_loss(
    ...
    if expert.beam_size > 1:
        import sys
        sys.exit("ImitKD  with beam size > 1 is not supported")
    with torch.no_grad():
        expert_preds = []
...
@register_criterion(
    "imit_kd_beam", dataclass=ImitKDConfig
)
class ImitKDBeam(FairseqCriterion):
    def __init__(
            self,
            task,
            expert,
        ...
        self.expert = SequenceGenerator([self.expert], self.expert_vocab_tgt, beam_size=1)
