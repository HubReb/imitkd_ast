...
def get_loss_components(net_output, expert_reward_to_go, indices, current_reward, ats, metric_bleu):
    rse = []
...
class Aggrevate(FairseqCriterion):
        ...
        if valid:
            ...
            if self.metric_bleu:
                ...
                logging_output = {
                    ...
                    "bleu": logged_bleu,
                    "indicator_sum": indicator_sum,
                    "ter_diff": bleu_diff.data,
                    "ter_complete_student_continuation": bleu_for_complete_student_hypos.data,
                    "ter_complete_expert_continuation": bleu_for_complete_expert.data
                }
        else:
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, valid=False):
        if valid:
            ...
            return nll_loss, loss, logged_bleu, bleu_diff_log, student_actions_probs, indicator, complete_student_bleu.sum(), complete_expert_bleu.sum()
        else:
            with torch.no_grad():
                source_text, _ = self.transform_source_tokens_into_expert_voc(sample)
        ...
        return loss, bleu_diff_log, student_actions_probs, indicator, complete_student_bleu.sum(), complete_expert_bleu.sum()
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        ter_diff = sum(log.get("ter_diff", 0) for log in logging_outputs)
        indicator_sum = sum(log.get("indicator_sum", 0) for log in logging_outputs)
        ...
        if "bleu_diff" in logging_outputs[0].keys():
            metrics.log_scalar(
    ...
    def calculate_bleu(self, target, hypos):
        ...
        for i, hypo in enumerate(hypos):
            ...
            # self._scorer.score(utils.strip_pad(target[i, :], self.padding_idx).cpu()  , hypo[0]['tokens'].int().cpu(), order=4)
    ...
    def calculate_rewards(self, sample, model, expert_output_samples, hypos_up_to_t,  hypos_to_t, indices, ats, source_text):
        ...
        student_hypos = [[{"tokens": utils.strip_pad(hypo, self.dict.pad())}] for hypo in
                         hypos_to_t.cpu()]
        # TODO: SequenceGenerator sets model into eval() mode internally - fix it if regularization method dropout is ever used
        ...
        if self.complete_indicator:
            ...
            actions_prob, bleu_diff, _ = get_loss_components(net_output, rtg, indices, qt, ats, self.metric_bleu)
        else:
            actions_prob, bleu_diff, indicator = get_loss_components(net_output, rtg, indices, qt, ats, self.metric_bleu)
        # actions, bleu_diff, _ = get_loss_components(net_output, rtg, indices, qt, ats)
        return bleu_diff, actions_prob, indicator, q_t_T, r_e
