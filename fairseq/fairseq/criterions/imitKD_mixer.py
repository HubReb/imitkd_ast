...
def imit_kd_loss(
    ...
    lprobs_all = model.get_normalized_probs(model(**sample["net_input"]), log_probs=True)
    ...
    with torch.no_grad():
        ...
        expert_out_all = expert.get_normalized_probs(expert(**sample["net_input"]), log_probs=False)
        target = sample["target"]
        target = target.unsqueeze(-1)
        pad_mask = target.eq(ignore_index)
        expert_out_all.masked_fill_(pad_mask, 0.0)
    ...
    imit_kd_loss_for_sample = -((1 - beta) * imit_kd_loss_for_sample.sum() + beta * (expert_out_all * lprobs_all).sum())
...
@register_criterion(
    "imit_kd_mixer", dataclass=ImitKDConfig
)
class ImitKD(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, valid=False):
        if valid:
            ...
            loss, logged_bleu = valid_loss(lprobs, target, sample, model, self.dict, self.eval_bleu, self.padding_idx,
                                           reduce=reduce, tokenizer=self.tokenizer)
        else:
