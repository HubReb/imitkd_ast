...
def one_minus_one_nll(lprobs, target, predictions, rl_value_pos, rl_value_neg, ignore_index=None, reduce=True):
    if predictions.shape[1] > lprobs.shape[1]:
        predictions = predictions[:, :lprobs.shape[1]]      # at times, offline model predictions are longer than the now training model's ones
    ...
    if ignore_index is not None:
        ...
        pad_mask_preds = predictions.eq(ignore_index)
        total_rewards.masked_fill_(pad_mask_preds, 0.0)
        predicted_class_probs.masked_fill_(pad_mask_preds, 0.0)
    else:
...
@register_criterion("rl_sampled_offline")
class RLSampledOffline(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def add_args(parser):
        ...
        # fmt: off
        parser.add_argument('--rl_value_pos', default=5., type=float, metavar='R',
    ...
    def get_lprobs_and_target(self, model, net_output, sample):
        ...
        try:        # validation steps have no predictions
            preds = sample["prediction"]
        except KeyError:
            preds = lprobs.argmax(-1) # greedy for simplicity
        if self.ignore_prefix_size > 0:
            if getattr(lprobs, "batch_first", False):
                lprobs = lprobs[:, self.ignore_prefix_size :, :].contiguous()
                target = target[:, self.ignore_prefix_size :].contiguous()
                preds = preds[:, self.ignore_prefix_size :].contiguous()
            else:
                raise ValueError(
                    "batch must be first"
                )
        return lprobs, target, preds

    def compute_loss(self, model, net_output, sample, reduce=True):
        lprobs, target, preds = self.get_lprobs_and_target(model, net_output, sample)
