...
def imit_kd_loss(
    ...
    lprobs_original = model.get_normalized_probs(model(**sample["net_input"]), log_probs=True)
    sample["net_input"]["src_tokens"] = source_text.cuda()
    sample["net_input"]["src_lengths"] = source_lengths
    with torch.no_grad():
        ...
        expert_out_original = expert.get_normalized_probs(expert(**sample["net_input"]), log_probs=True)
        preds_original = expert_out_original.argmax(-1)
    if preds.dim() == lprobs.dim() - 1:
        preds = preds.unsqueeze(-1)
    if preds_original.dim() == lprobs_original.dim() - 1:
        preds_original = preds_original.unsqueeze(-1)
    imit_kd_loss_for_sample = lprobs.gather(dim=-1, index=preds)
    kd_loss_original = lprobs_original.gather(dim=-1, index=preds_original)
    if ignore_index is not None:
        ...
        pad_mask = sample["target"].unsqueeze(-1).eq(ignore_index)
        kd_loss_original.masked_fill_(pad_mask, 0.0)
    imit_kd_loss_for_sample = -((1 - beta) * imit_kd_loss_for_sample.sum() + (beta * kd_loss_original).sum())
...
@register_criterion(
    "imit_kd_mixer_nll", dataclass=ImitKDConfig
)
