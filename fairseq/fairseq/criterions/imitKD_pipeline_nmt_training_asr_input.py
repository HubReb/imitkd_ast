...
@dataclass
class ImitKDPipelineTrainingAsrInputConfig(FairseqDataclass):
    expert: str = field(
...
@register_criterion(
    "imit_pipeline_nmt_training_asr_input", dataclass=ImitKDPipelineTrainingAsrInputConfig
)
class ImitKDPipelineTrainingAsrInput(FairseqCriterion):
    def __init__(
            self,
            task,
            expert,
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, valid=False):
        if valid:
            ...
            loss = imit_kd_loss(sample, model, self.expert, self.pad_idx)
        return loss
