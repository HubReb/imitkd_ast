...
class Difference(FairseqCriterion):
    def __init__(
        ...
        self.student_scorer = BleuScorer(self.dict.pad(), self.dict.eos(), self.dict.unk())
        self.expert_scorer = BleuScorer(self.expert_vocab_tgt.pad(), self.expert_vocab_tgt.eos(), self.expert_vocab_tgt.unk())
    ...
    def add_bleu_score_to_sample(self, sample, hypos, partial_hypos, expert_hypos):
        ...
        for i, hypos_i in enumerate(hypos):     # iterate over dataset
            ...
            h_s = self.dict.string(utils.strip_pad(hypos_i[0]['tokens'].int().cpu(), self.dict.pad()), bpe_symbol='sentencepiece')
            cosine_student = get_cosine_sim(ref_string, h_s)
            h = self.dict.encode_line(h_s, add_if_not_exist=False)
            ...
            # use +1 smoothing for sentence BLEU
            bleu_student_partial_hypo = self.student_scorer.score(r, h_partial)
            ref = self.expert_vocab_tgt.encode_line(
                    self.bpe.apply([ref_string])[0], add_if_not_exist=False, append_eos=False
            )
            h_e = self.expert_vocab_tgt.string(utils.strip_pad(expert_hypos[i].int().cpu(), self.expert_vocab_tgt.pad()), bpe_symbol='fastBPE')
            cosine_sim_expert = get_cosine_sim(ref_string, h_e)
            # print(cosine_student, cosine_sim_expert)
            h = self.expert_vocab_tgt.encode_line(h_e, add_if_not_exist=False)
            bleu_expert_hypo = self.expert_scorer.score(ref, h)
            reward_expert.append(cosine_sim_expert)
            reward_student.append(cosine_student)
            if cosine_student > cosine_sim_expert:
                # print(cosine_student, cosine_sim_expert)
                # print("reference: ", ref_string)
                # print("student: ", h_s)
                # print("expert: ", h_e)
...
def get_cosine_sim(str1, str2): 
    ...
    v1 = vectorizer.transform([str1])
    v2 = vectorizer.transform([str2])
