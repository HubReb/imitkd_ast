...
@register_criterion(
    "imit_asr_input_nmt_controlled", dataclass=ImitKDConfig
)
class ImitKD(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, valid=False):
        ...
        if valid:
            ...
            sample, asr_transcriptions, source_lengths = self.generate_imit_batch(model, sample, source_text, source_lengths)
    ...
    def generate_imit_batch(self, student: Callable, sample: Dict, source_text, source_lengths) -> Tuple[Dict, torch.IntTensor, List[int]]:
        """
        ...
        with torch.no_grad():
            ...
            source_text = source_text.data.tolist()
            ...
            for i, h in enumerate(student_hypos):
                if samp_mask[i]:
                    ...
                    transcriptions[i] = torch.tensor(source_text[i])
                    lengths = source_lengths[i]
