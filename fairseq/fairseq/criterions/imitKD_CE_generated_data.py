...
class ImitKDGenerated(FairseqCriterion):
    ...
    def generate_imit_batch(self, student: Callable, sample: Dict, source_text, source_len) -> Dict:
        """
        ...
        with torch.no_grad():
            ...
            source_audio = copy.deepcopy(sample["net_input"]["src_tokens"])
            source_audio_length = copy.deepcopy(sample["net_input"]["src_lengths"])
            sample["net_input"]["src_tokens"] = source_text.cuda()
            sample["net_input"]["src_lengths"] = torch.tensor(source_len)
            max_length = max(
                [len(i) for i in sample["target"]])  # let's avoid blowing up the GPU RAM, shall we?
            expert_generator = SequenceGenerator([self.expert], self.dict, beam_size=5, max_len=max_length)
            expert_hypos = expert_generator._generate(sample)
            for i, h in enumerate(expert_hypos):
            ...
            sample["net_input"]["src_tokens"] = source_audio
            sample["net_input"]["src_lengths"] = source_audio_length
