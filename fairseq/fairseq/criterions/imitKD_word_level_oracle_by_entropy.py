...
@register_criterion(
    "imit_kd_beta_word_level_oracle_by_entropy", dataclass=ImitKDConfig
)
class ImitKD(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True, valid=False):
        if valid:
            ...
            sample_s = copy.deepcopy(sample)
            sample_s["net_input"].pop("src_text", None)
            generated_dataset = self.generate_with_beta_function_word_level_oracle(model, sample_s, source_text, source_lengths)
            loss = imit_kd_loss(
                generated_dataset,
                model,
                self.expert,
    ...
    def generate_with_beta_function_word_level_oracle(self, student, sample, source_text, source_lengths):
        with torch.no_grad():
            student = student.eval()
            output_tokens = sample["net_input"]["prev_output_tokens"].data.tolist()
            original_output_tokens = sample["net_input"]["prev_output_tokens"].clone()
            ...
            for i, hypo in enumerate(hypos):
                ...
                new_output_tokens = torch.tensor(padded_hypothesis, device=torch.device('cuda'))
                output_tokens[i] = new_output_tokens[:]
            sample["net_input"]["prev_output_tokens"] = torch.stack(output_tokens)
            probs_hypothesis = student.get_normalized_probs(student(**sample["net_input"]), log_probs=False).cpu()
            entropies = entropy(probs_hypothesis, base=2, axis=-1)
            #print(entropies.mean(axis=-1), entropies.std(axis=1))
            max_entropies_indices = torch.tensor([entropies > entropies.mean()]).view(entropies.shape)
            sample_expert = {
                "id": sample["id"],
                "net_input": {
                    "src_tokens": source_text.cuda(),
                    "src_lengths": source_lengths,
                    "prev_output_tokens": original_output_tokens
                },
            ...
            expert_logits = torch.argmax(self.expert.get_normalized_probs(self.expert(**sample_expert["net_input"]), log_probs=True), axis=-1)
            #print(expert_logits.shape, max_entropies_indices.shape)
            for i, tokens in enumerate(output_tokens):
                output_tokens[i] = torch.tensor([expert_logits[i][j] if max_entropies_indices[i][j] else output_token for j, output_token in enumerate(tokens)], device="cuda")
                #output_tokens[i][max_entropies_indices[i]] = expert_logits[i][max_entropies_indices[i]]
            sample["net_input"]["prev_output_tokens"] = torch.stack(output_tokens)
            student.train()
        return sample
