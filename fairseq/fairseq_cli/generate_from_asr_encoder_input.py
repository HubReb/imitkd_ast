...
def _main(cfg: DictConfig, output_file):
    ...
    sp_model.Load("/home/rebekka/t2b/Projekte/ma/knn_ast_kd_nmt/fairseq/examples/speech_to_text/covost/en/spm_bpe8000_st_en_de.model")
    def decode_fn(x):
        if bpe is not None:
            x = bpe.decode(x)
    ...
    for sample in progress:
        ...
        for i, line in enumerate(source_text):
            if type(line) == list:
                ...
                if line is None:
                    print(task.tgt_dict.string(sample['target'][i], bpe_symbol="sentencepiece"))
        ...
        for b, hyp in enumerate(hypos):
            ...
            source_texts.append(expert_vocab_src.encode_line(bpe_expert_src.apply(
                [task.tgt_dict.string(utils.strip_pad(hyp[0]["tokens"].clone().detach(), expert_vocab_src.pad()), bpe_symbol="sentencepiece")])[0],
