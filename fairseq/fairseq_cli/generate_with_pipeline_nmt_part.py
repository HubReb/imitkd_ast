...
def _main(cfg: DictConfig, output_file):
    ...
    bpe_expert_src = fastBPE.fastBPE(
        ...
        "/home/rebekka/t2b/Projekte/ma/knn_ast_kd_nmt/fairseq/examples/speech_to_text/wmt19.en-de.joined-dict.ensemble/dict.en.txt"
    )
    expert_vocab_tgt = Dictionary.load("/home/rebekka/t2b/Projekte/ma/knn_ast_kd_nmt/fairseq/examples/speech_to_text/wmt19.en-de.joined-dict.ensemble/dict.de.txt")
    expert_vocab_src = Dictionary.load("/home/rebekka/t2b/Projekte/ma/knn_ast_kd_nmt/fairseq/examples/speech_to_text/wmt19.en-de.joined-dict.ensemble/dict.en.txt")
    expert_generator = SequenceGenerator([asr_model], expert_vocab_src, beam_size=5)
    ...
    for sample in progress:
        ...
        for b, hyp in enumerate(hypos):
            source_texts.append(expert_vocab_src.encode_line(bpe_expert_src.apply(
                [task.tgt_dict.string(utils.strip_pad(hyp[0]["tokens"].clone().detach(), expert_vocab_src.pad()),
                                      bpe_symbol="fastBPE")])[0],
                                                             add_if_not_exist=False,
                                                             append_eos=True)
                                )
        nmt_input = collate_tokens(
            source_texts,
